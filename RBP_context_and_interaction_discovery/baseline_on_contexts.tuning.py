...
if len(sys.argv) != 11:
    sys.exit("Enter:\n (1) file containing all sequences\n (2) list of baseline kmer sizes (comma-separated)\n \
(3) list of baseline step sizes (comma-separated)\n (4) context target size\n (5) context target step\n \
(6) context kmer size\n (7) context kmer step\n (8) number of context kmers on each side of target\n \
(9) threshold for relabeling\n (10) number of relabeling iterations.")

all_seq_file = sys.argv[1]
baseline_kmer_size_list = [int(x) for x in sys.argv[2].split(",")]
baseline_step_size_list = [int(x) for x in sys.argv[3].split(",")]
context_target_size = int(sys.argv[4])
context_target_step = int(sys.argv[5])
context_kmer_size = int(sys.argv[6])
context_kmer_step = int(sys.argv[7])
num_ctxt = int(sys.argv[8])
threshold = float(sys.argv[9])
num_iterations = int(sys.argv[10])
...
if __name__ == "__main__":
    ...
    # ---------------------------------------- SELECT HYPERPARAMETERS FOR THE MODELS ----------------------------------------

    baseline_model_type = "dense"
    baseline_embedding_type = "BoW"

    contexts_model_type = "dense_embed"
    contexts_embedding_type = "embedding"

    hyperparam_ranges = {
        "hidden_shape": [16, 32, 64, 128, 256, 512, 1024, 2048],
        "embedding_dim": [8, 16, 32, 64, 128],
        "num_filters": [2, 3, 4, 8],
        "kernel_length": [2, 3, 4, 5, 6, 7, 8, 9, 10],
    ...
    hidden_shape = random.sample(hyperparam_ranges["hidden_shape"], 1)[0]
    embedding_dim = random.sample(hyperparam_ranges["embedding_dim"], 1)[0]
    num_filters = random.sample(hyperparam_ranges["num_filters"], 1)[0]
    kernel_length = random.sample(hyperparam_ranges["kernel_length"], 1)[0]
    epochs = random.sample(hyperparam_ranges["epochs"], 1)[0]
    batch_size = random.sample(hyperparam_ranges["batch_size"], 1)[0]

    # Print hyperparameters to a file
    hyperparameters_df = pd.DataFrame({"baseline_model_type": [baseline_model_type], \
                                        "baseline_embedding_type": [baseline_embedding_type], \
                                        "contexts_model_type": [contexts_model_type], \
                                        "contexts_embedding_type": [contexts_embedding_type], \
    ...
    hyperparameters_df.transpose().to_csv("hyperparameters.tsv", sep = "\t", header = False)

    # Set up summary output file
    summary_output_file = "run_summary.tsv"
    with open(summary_output_file, 'a') as f:
        print("----- Model hyperparameters -----", file = f)
        ...
        print("\tHidden shape:" + "\t" + str(hidden_shape), file = f)
        print("\tEmbedding dimension:" + "\t" + str(embedding_dim), file = f)
        print("\tNumber of filters: " + str(num_filters), file = f)
        print("\tKernel length:" + "\t" + str(kernel_length), file = f)
        print("\tNumber of epochs:" + "\t" + str(epochs), file = f)
    ...
    baseline_model.hidden_shape = hidden_shape
    baseline_model.out_shape = 1
    baseline_model.embedding_dim = embedding_dim
    baseline_model.num_filters = num_filters
    baseline_model.kernel_length = kernel_length
    baseline_model.kernel_stride = 1
    baseline_model.pool_size = 2
    baseline_model.epochs = epochs
    baseline_model.batch_size = batch_size
    ...
    contexts_model.hidden_shape = hidden_shape
    contexts_model.out_shape = 1
    contexts_model.embedding_dim = embedding_dim
    contexts_model.num_filters = num_filters
    contexts_model.kernel_length = kernel_length
    contexts_model.kernel_stride = 1
    contexts_model.pool_size = 2
    contexts_model.epochs = epochs
    contexts_model.batch_size = batch_size
    ...
    # # Write contexts to a fasta file
    # test_preprocessor.write_ctxt_to_fasta(baseline_on_contexts_test_output_df, threshold)
    ...
    #     contexts_model.refit(X_train_contexts, y_train_contexts_relabeled)

    #     # Make predictions with re-trained model
    #     predictions_train_contexts_relabeled = contexts_model.predict(X_train_contexts)
    #     predictions_test_contexts_relabeled = contexts_model.predict(X_test_contexts)
    ...
    #                 contexts_relabeled_train_aupr = contexts_model.evaluate_plus(X_train_contexts, \
    ...
    #                 contexts_relabeled_test_aupr = contexts_model.evaluate_plus(X_test_contexts, \
    ...
    #     # Write contexts to a fasta file
    #     test_preprocessor.write_ctxt_to_fasta(contexts_relabeled_test_output_df, threshold)
    ...
    #                 contexts_relabeled_train_bag_aupr = contexts_model.evaluate_bag(y_train_split_seq, contexts_relabeled_train_output_df, \
    #                                                                                 threshold, method, \
    ...
    #                 contexts_relabeled_test_bag_aupr = contexts_model.evaluate_bag(y_test_split_seq, contexts_relabeled_test_output_df, \
    #                                                                                 threshold, method, \
    ...
    #     # contexts_model.save_model(contexts_relabeled_model_filename)

    #     # Save object
    #     contexts_model.save_object("contexts_model.iteration" + str(i + 1) + ".obj")
